...
if ( ! function_exists('maintain_ssl'))
{
	function maintain_ssl($maintain = FALSE, $port = 443)
	{
		...
		if ($maintain)
		{
			...
			// form temp base url
			$temp_base_url = 'https://'.$domain[0].($port != 443 ? ':'.$port : '');
			// replace segments
            ...
            if (empty($_SERVER['HTTPS']))
                header('Location: ' . $temp_base_url . $_SERVER['PATH_INFO']);
		}

		// if don't maintain but SSL is on -OR- maintain but SSL isn't on, correct by redirect
		// if ($maintain && (empty($_SERVER['HTTPS']) || !isset($_SERVER['HTTPS'])))
		// {
		...
		// 	$CI->load->helper('url');
		// 	$url = current_url();
		// 	$url = substr(current_url(), strpos($url, '://') + 3);
		// 	header('Location: '.'https://'.$url.(empty($_SERVER['QUERY_STRING']) ? '' : '?'.$_SERVER['QUERY_STRING']));
		// }
	}
}
