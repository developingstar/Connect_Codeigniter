...
class Ownservices extends CourierController {
...
public function create_service() {
 ...
 if ($courier_id == NULL) {
   $couriers_id = $this->session->userdata("courier_id");
   $courier = $this->couriers_model->get_by_id($couriers_id);
   if (!$courier) {
    $error = TRUE;
  ...
  if ($items) {
   foreach ($items as $item) {
    switch ($item->location) {
     ...
     break;
     case 16: $item->location_name = "Collect and deliver on the same day";
     break;
     case 17: $item->location_name = "Collect the day before delivery";
     break;
     case 18: $item->location_name = "Collect back on next business day";
     break;
     case 19: $item->location_name = "Collect back on same day as delivery";
     break;
     case 20: $item->location_name = "Collect back within a week";
     break;
     case 25: $item->location_name = "Appointed Service";
     break;
   }
 }
 $result['items'] = $items;
