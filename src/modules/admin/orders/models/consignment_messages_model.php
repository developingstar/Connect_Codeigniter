<?php
  ...
  class Consignment_messages_model extends CI_Model {
       ...
       public function get_last_comment($order_id) {
            $query = 'SELECT C.comment_id as msg_id,C.comment as msg,NULL as reply, DATE_FORMAT(C.time,"%d %b %Y %h:%i %p") as time ,C.time as time2,UNIX_TIMESTAMP(C.time) AS t1,NULL as replytime,"comment" as type,NULL as courier '
       ...
       public function getmessages($order_id) {
            $query = 'SELECT * FROM (SELECT C.comment_id as msg_id,C.comment as msg,NULL as reply,UNIX_TIMESTAMP(C.time) AS t1, DATE_FORMAT(C.time,"%d %b %Y %h:%i %p") as time , C.time as time2,NULL as replytime,"comment" as type,NULL as courier '
                    . 'FROM consignment_comments as C '
                    . 'WHERE order_id = ' . $order_id . ' UNION '
                    . 'SELECT M.message_id as msg_id, M.question as msg, M.reply,UNIX_TIMESTAMP(M.created_date)  AS t1, DATE_FORMAT(M.created_date,"%d %b %Y %h:%i %p") as time,M.created_date as time2, DATE_FORMAT(M.updated_date,"%d %b %Y %h:%i %p") as replytime, "message" as type,C.company_name as courier '
                    . 'FROM consignment_messages as M '
                    . ' JOIN couriers as C ON C.courier_id=M.courier_id '
                    . 'WHERE job_id = ' . $order_id . ' AND M.courier_id <> 0 UNION '
                    . 'SELECT M.message_id as msg_id, M.question as msg, M.reply,UNIX_TIMESTAMP(M.created_date)  AS t1, DATE_FORMAT(M.created_date,"%d %b %Y %h:%i %p") as time,M.created_date as time2, DATE_FORMAT(M.updated_date,"%d %b %Y %h:%i %p") as replytime, "message" as type,C.email as courier '
                    . 'FROM consignment_messages as M '
