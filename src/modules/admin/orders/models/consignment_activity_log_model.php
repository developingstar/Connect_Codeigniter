...
class Consignment_activity_log_model extends MY_Model {
...
public function getloglist_by_orderid($order_id, $perpage, $search, $start) {
  $this->db->select('id, order_id, activity, (UNIX_TIMESTAMP(time)*1000) as time', FALSE);
//            $this->db->select('id, order_id, activity, DATE_FORMAT(time,"%d %b %Y %h:%i %p")as time', FALSE);
  $this->db->from('consignment_activity_log');
  $this->db->where("order_id", $order_id)->order_by('time',"DESC");
...
public function saveDeletedFiles($order_id, $username, $filename){
  $data = array(
    'order_id' => $order_id,
    'activity' => $username." has removed the attachment ".$filename." from the delivery",
    'category' => 1
    ); 
  $this->insertRow($data, 'consignment_activity_log');
}

public function saveAddedFiles($order_id, $username, $filename){
  $data = array(
    'order_id' => $order_id,
    'activity' => $username." has uploaded an attachment ".$filename." to the delivery",
    'category' => 1
    ); 
  $this->insertRow($data, 'consignment_activity_log');
}
