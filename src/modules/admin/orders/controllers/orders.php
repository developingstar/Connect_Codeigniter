<?php
  ...
  class Orders extends MY_Controller {
       ...
       function __construct($rootObject = 'orders') {
            parent::__construct();
       ...
       function uploadattachments() {
            ...
            if (isset($_FILES['file'])) {
                 ...
                 for ($i = 0; $i < $cpt; $i++) {
                      $tcount = $this->consignment_attachment_model->attachmentCountsByOrderId($order_id);
                      if ($tcount > 15) {
                           $data = array('status' => FALSE, 'error' => 'Only 15 files can be attached');
                           break;
                      }
                      $_FILES['attachment']['name'] = $files['file']['name'][$i];
                      $_FILES['attachment']['type'] = $files['file']['type'][$i];
                      $_FILES['attachment']['tmp_name'] = $files['file']['tmp_name'][$i];
                      $_FILES['attachment']['error'] = $files['file']['error'][$i];
                      $_FILES['attachment']['size'] = $files['file']['size'][$i];
                      $name_arr = explode('.', $_FILES['attachment']['name']);
                      $ext = end($name_arr);
                      $filename = md5($_FILES['attachment']['name'] . time()) . '.' . $ext;
                      if ($this->s3->putObjectFile($_FILES['attachment']['tmp_name'], $this->bucket, $this->rootObject . $filename, S3::ACL_PUBLIC_READ)) {
                           $files = array('code' => 200, 'name' => $_FILES['attachment']['name'], 'file_name' => $filename, 'path' => "http://$this->bucket.s3.amazonaws.com/" . $this->rootObject . $filename, 'type' => $_FILES['attachment']['type'], 'size' => $_FILES['attachment']['size']);

                           $attachment = array(
                               'order_id' => $order_id,
                               'path' => "http://$this->bucket.s3.amazonaws.com/" . $this->rootObject . $filename,
                               'name' => $filename
                           );
                           ...
                           $user = $this->consignment_activity_log_model->getOneWhere($where, 'member');
                           $username = $user->username;
                           $this->consignment_activity_log_model->saveAddedFiles($order_id, $username, $file);
                           $data = array('status' => true, 'error' => '', 'data' => '','path'=>$attachment['path']);
                      } else {
       ...
       public function downloadAttachment($id) {
            ...
            $file = explode('/', $attach->path);
            $filename = end($file);
            $object = $this->s3->getObjectInfo($this->bucket, $this->rootObject . $filename);
            if ($object) {
                 $savePath = 'filebox/aws_cache';
                 if (!file_exists($savePath)) {
                      @mkdir($savePath, 0777, true);
                 }
                 $file = $this->s3->getObject($this->bucket, $this->rootObject . $filename, $savePath . '/' . $filename);
                 ...
                 header('Content-Length: ' . filesize($savePath . '/' . $filename));
                 readfile($savePath . '/' . $filename);
                 @unlink($savePath . '/' . $filename);
                 exit;
            } else {
                 show_error('The file you requested could not find on server.', 404, 'File Not Found');
            }
       }
       ...
       public function deleteAttach() {
            ...
            $order = $this->consignment_activity_log_model->getOneWhere(array('id' => $id), 'consignment_attachments');
            $order_id = $order->order_id;
            $this->consignment_activity_log_model->saveDeletedFiles($order_id, $username, $filename);

            // Delete from attachments record
            $this->orders_model->deleteRow($id, 'id', 'consignment_attachments');

            $object = $this->s3->getObjectInfo($this->bucket, $this->rootObject . $filename);
            if ($object) {
                 $this->s3->deleteObject($this->bucket, $this->rootObject . $filename);
       ...
       public function printOrder($order_id) {
            ...
            if (!file_exists(base_url('filebox/ciqrcode/' . $order->public_id . '.png'))) {
                 $this->generate_ciqrcode($order->public_id);
            }

            if (!file_exists(base_url('filebox/barcode/consignment_document_' . $order->public_id . '.png'))) {
                 $this->generate_barcode($order->public_id);
            }
       ...
       function getholidays() {
            ...
            $count = $this->orders_model->getHolidays($ordersData->date);
            echo json_encode(array('days' => $count + intval($days)));
            exit();
       }
...
//debug($where);
       ...
       public function messageslist_json() {
            ...
            foreach ($messages AS $message) {
                 $message->msgtime = strtotime($message->time);
                 $message->t2 = date("r", $message->t1);
                 $retmsg[] = $message;
            }
       ...
       public function saveOrder() {
            ...
            $collect_back_item = 0;
            if (isset($orderdata->collect_back_item) && $orderdata->collect_back_item!='') {
                 $collect_back_item = (int) $orderdata->collect_back_item;
...
//            if (isset($orderdata->is_d_restricted_area->clocation) && $orderdata->is_d_restricted_area->clocation == true) {
...
//            }
            ...
            $is_c_restricted_area = 0;
            if (isset($orderdata->is_d_restricted_area->clocation) && is_numeric($orderdata->is_d_restricted_area->clocation)) {
                 $is_c_restricted_area = intval($orderdata->is_d_restricted_area->clocation);
            }


            $is_d_restricted_area = 0;
            if (isset($orderdata->is_d_restricted_area->loc)) {
                 $is_d_restricted_area = intval($orderdata->is_d_restricted_area->loc);
            }

            $c_appointment = 0;
            if (isset($orderdata->is_d_restricted_area->appointment1)) {
                 $c_appointment = intval($orderdata->is_d_restricted_area->appointment1);
            }

            $d_appointment = 0;
            if (isset($orderdata->is_d_restricted_area->appointment)) {
                 $d_appointment = intval($orderdata->is_d_restricted_area->appointment);
            }

            $collect_back = 0;
            if (isset($orderdata->is_d_restricted_area->collect_back) && is_numeric($orderdata->is_d_restricted_area->loc)) {
                 $collect_back = intval($orderdata->is_d_restricted_area->collect_back);
            }
...
//            }
            ...
            $payment_modedata = $orderdata->payment_mode;
            if (!$is_edit) {
                 if ($payment_mode->value == '_1__' || $payment_mode->value == '1___') {
                      $p_account = $this->orders_model->get_account_money($payment_modedata->id);
                      if ($p_account->credit < $price) {
                           $error = true;
                           $errors['threshold'] = "Insufficient Credit Amount";
                           $error_part3 = 1;
                      }
                 }
            }
            ...
            if (!$error) {
                 ...
                 if ($is_edit) {
                      $order_data = array(
                          ...
                          'remarks' => $remarks,
                          'c_appointment' => $c_appointment,
                          'd_appointment' => $d_appointment,
                          'collect_back' => $collect_back_item,
                      ...
                      if ($insert_id) {
                           ...
                           if ($payment_mode->value == '_1__' || $payment_mode->value == '1___') {
                                $p_account = $this->orders_model->get_account_money($payment_mode->id);
                                $balance = floatval($p_account->credit) - floatval($price);
                                $this->orders_model->update_account_money($payment_mode->id, $balance);
                           }
                           ...
                           $this->generate_ciqrcode($public_id);
                           if ($is_third_party) {
                                $third_party_info = array(
                                    'email' => $third_party_email,
                                    'user_id' => $user_id
       ...
       private function reciept($public_id = 0) {
            ...
            if ($order_id == 0) {
                 ...
                 return $this->load->view('couriers/reciept/reciept_popup', array('order' => $order, 'customer' => $customer, 'organization' => $org, 'item_type' => $item_type), true);
            }
       }
       ...
       public function view_order($order_id) {
            ...
            $data['order_receipt'] = NULL;
            if (($order->payment_type == 1 || $order->payment_type == 2) && $order->consignment_status_id == 5) {
                 $data['order_receipt'] = $this->reciept($order->public_id);
            }
...
//            $datetime3 = date_create(date('Y-m-d'));
            ...
            $locations = array();
            $location = NULL;
            if (isset($getdata->location->loc) && is_numeric($getdata->location->loc)) {
                 $location = $getdata->location->loc;
            }

            $clocation = NULL;

            if (isset($getdata->location->clocation) && is_numeric($getdata->location->clocation)) {
                 $clocation = $getdata->location->clocation;
            }

            $collect_back = null;
            if ($getdata->location->collect_back) {
                 $collect_back = intval($getdata->location->collect_back);
                 $locations[] = intval($getdata->location->collect_back);
            }
            if ((isset($getdata->location->appointment) && $getdata->location->appointment == APPOINTED_SERVICE) || ($getdata->location->appointment1 == APPOINTED_SERVICE && isset($getdata->location->appointment1))) {
                 $locations[] = APPOINTED_SERVICE;
            }
            ...
            foreach ($services as $service) {
                 ...
                 if ($svalue[0] && $svalue[0] != 0) {
                      if ($org_id != null && $org_id != -1) {
                           $approved_accounts = array(); //$this->payment_accounts_model->get_approved_accounts($org_id, 2);
                      } else {
                           $approved_accounts = $this->payment_accounts_model->get_approved_accounts($user, 1);
                      }
                      foreach ($approved_accounts as $acc) {
                           $s_options[] = array('id' => $acc->id, 'type' => lang('credit_terms_direct'), 'value' => "1___", 'name' => $acc->account_name, 'credit' => $acc->credit);
                      }
                 }
       ...
       public function approve_price($public_id = 0, $approve = true) {
            ...
            if ($approve) {
                 ...
                 if ($this->orders_model->updateOrder($data, $job_id)) {
                      $this->consignment_activity_log_model->add_activity(array(
                          "order_id" => $job_id,
                          "activity" => "Price Change( $" . $price . " > $" . $pricechange . ") request rejected by user"
                      ));
