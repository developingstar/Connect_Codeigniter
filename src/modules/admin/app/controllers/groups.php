	...
	public function save() {
		...
		if (count($errors) == 0) {
			if ($is_edit) {
				...
				if (isset($result)) {
//                      $org = $this->organisation_model->getName($org_id);
//                      $user = $this->account_model->get_by_id($this->session->userdata("user_id"));
//                      $log = array(
//                          'group_ref' => $result,
//                          'action' => 1,
//                          'remark' => "New group '" . $group_name . "' added to organisation '" . $org->name . "' by '" . $user->username . "'",
//                          'update_by' => $this->session->userdata("user_id")
//                      );
//                      $this->log_model->addLog($log);
					$result_array['status'] = 1;
	...
	public function update_group() {
		...
		$group_data = json_decode($postdata);
		$result = array();
		if ($group_data->group_id == $this->session->userdata('user_id')) {
