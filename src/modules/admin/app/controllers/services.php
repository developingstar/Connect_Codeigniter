<?php
  ...
  class Services extends MY_Controller {
       ...
       public function update_service() {
            ...
            $service_data = json_decode($postdata);
            $result = array();
            if ($service_data->service_id == $this->session->userdata('user_id')) {
            ...
            if (isset($service_data->service_id) && !empty($service_data->service_id)) {
                 ...
                 if ($result) {
                      ...
                      $message = array(
                          'title' => '',
                          'name' => $to_name,
                          'content' => 'New Service \'' . $email->display_name . '\' Approved by  \'' . $org_name->name . '\' Admin',
                          'link_title' => '',
                          'link' => '',
                          'link2' => '');
            ...
            if (isset($service_data->service_id) && !empty($service_data->service_id)) {
                 ...
                 if ($result) {
                      ...
                      $message = array(
                          'title' => '',
                          'name' => $to_name,
                          'content' => '<p>New Service \'' . $email->display_name . '\' Rejected by  \'' . $org_name->name . '\' Admin</p>'
                          . '<p>' . $remark . '<p>',
                          'link_title' => '',
                          'link' => '',
                          'link2' => '');
       ...
       public function get_courier_profile() {
            ...
            $courrier_rating = $this->services_model->get_courier_rating($courier_id);
            if ($courrier_rating->count > 5) {
                 if ($courier->rating != 0) {
                      $courier->rating = ceil(($courier->rating * 0.8) + ($courrier_rating->sum / $courrier_rating->count) * 0.2);
                 } else {
                      $courier->rating = ceil($courrier_rating->sum / $courrier_rating->count);
                 }
            }
            if ($courier) {
                 $until = new DateTime();
                 ...
                 $ontime_deliveries = $this->services_model->get_ontimedeliveries_count($courier_id, $from_date);
                 $success = 0;
                 $failed = 0;
                 $cancelled = 0;
                 $bidding = 0;
                 ...
                 foreach ($orders as $order_cat) {
                      $total_deliveries = $total_deliveries + $order_cat->deliveries;
                      if ($order_cat->status != 7 && $order_cat->status != 8 && $order_cat->status != 9) {
                           $accepted = $accepted + $order_cat->deliveries;
                      }
                      switch ($order_cat->status):
                           case 6:$failed = $order_cat->deliveries;
                                break;
                           case 5:$success = $order_cat->deliveries;
                                break;
                           case 9:$bidding = $order_cat->deliveries;
                                break;
                           case 11:$cancelled = $order_cat->deliveries;
                                break;
                           case 19:$rejected = $order_cat->deliveries;
                                break;
                      endswitch;
                 }
                 ...
                 $success_rate = 0;
                 $reject_rate = 0;
                 $total = $total_deliveries - $cancelled - $bidding;
                 if ($total != 0) {
                      $success_rate = round(($success / $total) * 100);
                 }
                 if ($accepted != 0) {
                      $reject_rate = round((1-($rejected / $accepted)) * 100);
                 }
                 if($success!=0){
                      $ontime=  round(($ontime_deliveries/$success)*100);
                 }

                 if (strlen($courier->description) > 100) {
                      $description = substr($courier->description, 0, 100);
                      $description1 = substr($courier->description, 100);
                 } else {
                 ...
                 $result['courier'] = array(
                     'logo' => ($courier->logo !== NULL) ? $courier->logo . '?' . time() : base_url("resource/images/default_logo.png") . '?' . time(),
                     'name' => $courier->name,
                     'rating' => $courier->rating,
                     'description' => $description,
                     'description1' => $description1,
                     'phone' => $courier->phone,
                     'reg_address' => $courier->reg_address,
                     'insured_policy' => $courier->insured_policy,
                     'reg_no' => $courier->reg_no,
                     'completed' => $success,
                     'success_rate' => $success_rate . '%',
                     'del_time_rate' => $ontime . '%',
       ...
       public function get_service_info($id = 0) {
            ...
            if ($service) {
                 ...
                 $service_details['end_time'] = $service->end_time;
                 $service_details['working_days'] = implode(', ', $sdays);
                 $service_details['payment_term'] = $service->payment_terms;
                 $service_details['origin'] = '-';
                 $origin = $this->courier_service_model->get_service_origin($id);
                 if ($origin) {
                      $service_details['origin'] = $origin->country;
