...
//function to verify new member id not added before
       ...
       function getmemberslist_by_orgid($org_id, $perpage, $search, $start) {
            $this->db->select('member.id as id,member.email as Email,member_details.fullname as FullName,organization_members.org_id,organization_members.note as Note,organization_members.role_id as role,organization_members.status as Status,groups.id as group_id,groups.name as groupname');
       ...
       function get_detail_by_userid($user_id, $org_id) {
            $this->db->select('member.email,member_details.*,organization_members.*,country.country,groups.name as groupname,groups.id as group_id', FALSE);
            $this->db->from('organization_members');
       ...
       public function getallmemberaccounts_count($search) {
            if ($search != NULL) {
                 $this->db->where('(M.email LIKE \'%' . $search . '%\' OR MD.description LIKE \'%' . $search . '%\' OR MD.fullname LIKE \'%' . $search . '%\' OR O.name LIKE \'%' . $search . '%\')');
            }
            return $this->db->select('M.id')
                            ->from('member as M')
                            ->join('member_details as MD', 'M.id=MD.user_id', 'LEFT')
                            ->join('organization_members AS OM', "OM.user_id=M.id", "LEFT")
                            ->join('organizations AS O', "OM.org_id=O.id", "LEFT")
                            ->group_by('M.id')
                            ->get()
                            ->num_rows();
       }
       ...
       public function getallmemberaccounts($perpage, $search, $start) {
            ...
            $data = $this->db->select('M.id, email, root, M.status, language,fullname, '
                            . 'gender, C.country, picture, phone_no, fax_no, MD.description,UNIX_TIMESTAMP(M.createdon) AS createdtime')
                    ->from('member as M')
                    ->join('member_details as MD', 'M.id=MD.user_id', 'LEFT')
                    ->join('organization_members AS OM', "OM.user_id=M.id", "LEFT")
                    ->join('organizations AS O', "OM.org_id=O.id", "LEFT")
            ...
            foreach ($data as $value) {
                 $value->organization = $this->db->select('O.name')
                         ->from('organizations AS O')
                         ->join('organization_members AS OM', "OM.org_id=O.id")
                         ->where('OM.user_id', $value->id)
                         ->get()
                         ->result();
                 $retdata[] = $value;
            }
