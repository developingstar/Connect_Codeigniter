			...
			if ($this->hybridauthlib->providerEnabled($provider)) {
				...
				if ($service->isUserConnected()) {
					...
					if (!$user = $this->account_model->get_by_email($user_profile->email)) {
						...
						$user_id = $this->account_model->create($user_profile->firstName, $user_profile->email, NULL, false, 2);
						// Add user details (auto detected country, language, timezone)
						$this->account_details_model->update($user_id, array(
	...
	function facebook()
	{
		...
		if ($fb_user_id == 0) {
			redirect($this->facebook->getLoginUrl(array(
				'redirect_uri' => site_url('social/facebook'),
				'scope' => 'email, public_profile'
			)));
		}
		else
		{
			$fb_user = $this->facebook->api('/me?fields=id,email,name');
			if (!empty($fb_user['email'])) {
				...
				if (!$user = $this->account_model->get_by_email($f_user['email'])) {
					
					$user_id = $this->account_model->create($f_user['name'], $f_user['email'], NULL, false, 1);
					
					// Add user details (auto detected country, language, timezone)
					$this->account_details_model->update($user_id, array(
						'fullname' => $f_user['name']
					));
					$token = $this->token();
					$data['user_id'] = $user_id;
					$data['token'] = $token;
				...
				$data = array();
				$data['sign_in_error'] = lang('facebook_no_email');
