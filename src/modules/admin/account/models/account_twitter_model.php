...
class Account_twitter_model extends CI_Model {
	...
	{
		return $this->db->get_where('a3m_account_twitter', array('user_id' => $user_id))->result();
	}
	 ...
	 * @return void
	 */
	function insert($user_id, $twitter_id, $oauth_token, $oauth_token_secret)
	{
		$this->load->helper('date');

		if ( ! $this->get_by_twitter_id($twitter_id)) // ignore insert
		{
			$this->db->insert('a3m_account_twitter', array('user_id' => $user_id, 'twitter_id' => $twitter_id, 'oauth_token' => $oauth_token, 'oauth_token_secret' => $oauth_token_secret, 'linkedon' => mdate('%Y-%m-%d %H:%i:%s', now())));
			return TRUE;
		}
		return FALSE;
	}
