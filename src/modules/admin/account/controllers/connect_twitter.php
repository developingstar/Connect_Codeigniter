	...
	function index()
	{
		...
		if ($this->input->get('oauth_token'))
		{
			...
			if ($user = $this->account_twitter_model->get_by_twitter_id($twitter_info['id']))
			{
				...
				$user->user_id === $this->session->userdata('user_id') ? $this->session->set_flashdata('linked_error', sprintf(lang('linked_linked_with_this_account'), lang('connect_twitter'))) : $this->session->set_flashdata('linked_error', sprintf(lang('linked_linked_with_another_account'), lang('connect_twitter')));
			...
			else
			{
				...
				else
				{
					...
					$this->account_twitter_model->insert($this->session->userdata('user_id'), $twitter_info['id'], $twitter_token->oauth_token, $twitter_token->oauth_token_secret);
