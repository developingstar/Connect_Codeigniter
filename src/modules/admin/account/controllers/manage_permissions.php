<?php
  ...
  class Manage_permissions extends MY_Controller {
       ...
       public function updatePermissions() {
            ...
            //retrieve data from angularjs $http call
            $postdata = file_get_contents("php://input");
            $userinfo = json_decode($postdata);
            $errors = array();
            $id = $userinfo->permission_id;
            $name_taken = $this->name_check($userinfo->permission_key);
            ...
            if ((!empty($id) && strtolower($userinfo->permission_key) != strtolower($data['permission']->key) && $name_taken) || (empty($id) && $name_taken)) {
                 $errors['permission_key_error'] = lang('permissions_name_taken');
            } else {
                 // Create/Update role
                 $attributes = array();
                 ...
                 // Not allowed to update keys for System Permissions
                 if (!$data['is_system']) {
                      $attributes['key'] = $userinfo->permission_key ? $userinfo->permission_key : NULL;
                 }

                 $attributes['description'] = $userinfo->permission_description ? $userinfo->permission_description : NULL;
                 $id = $this->acl_permission_model->update($id, $attributes);


                 // Check if the permission should be disabled
                 if ($this->authorization->is_permitted('delete_permissions')) {
                      if ($userinfo->manage_permission_ban) {
                           $this->acl_permission_model->update_suspended_datetime($id);
                      } elseif ($userinfo->manage_permission_unban) {
                           $this->acl_permission_model->remove_suspended_datetime($id);
                      }
                 }
                 ...
                 $perms = array();
                 foreach ($data['roles'] as $role) {
                      if ($userinfo->role_permission_{$role->id}) {
                           $this->rel_role_permission_model->update($role->id, $id);
                      } else {
                           $this->rel_role_permission_model->delete($role->id, $id);
                      }
