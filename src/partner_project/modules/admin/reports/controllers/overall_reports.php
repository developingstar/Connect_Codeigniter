<?php
  ...
  class Overall_reports extends MY_Controller {

       function __construct() {
            parent::__construct();
            $this->load->model(array('overall_reports_model', 'app/services_model', 'app/groups_model', 'reports_model'));
       }
       ...
       private function get_export_data() {
            ...
            switch ($type) {
                 case 'hourly':
                      $title = "Overall Performance Details per Hour " . $start_date . " - " . $end_date . "";
                      ...
                      $reportinfo = $this->overall_reports_model->overall_performance_hour_info($start_date, $end_date, $org_id);
                      break;
                 case 'day':
                      $title = "Overall Performance Details per Day " . $start_date . " - " . $end_date . "";
                      $time = strtotime($start_date);
                      ...
                      $reportinfo = $this->overall_reports_model->overall_performance_day_info($start_date, $end_date, $org_id);
                      break;
                 case "week":
                      $title = "Overall Performance Details per Week " . $start_date . " - " . $end_date . "";
                      $time = strtotime($start_date);
                      ...
                      $reportinfo = $this->overall_reports_model->overall_performance_week_info($start_date, $end_date, $org_id);
                      break;
                 case "month":
                      $title = "Overall Performance Details per Month " . $start_date . " - " . $end_date . "";
                      $time = strtotime($start_date);
                      ...
                      $reportinfo = $this->overall_reports_model->overall_performance_month_info($start_date, $end_date, $org_id);
                      break;
            }
       ...
       public function get_overall_performance_pdf() {
            $performance = $this->get_export_data();
            $filename = "overall_performance_details.pdf";
            export_as_pdf($filename, $performance);
       }

       public function get_overall_performance_excel() {
            $performance = $this->get_export_data();
            $filename = "overall_performance_details";
            export_as_excel($filename, $performance);
       }

       public function getoverallperformancedata() {
            $result = array();
            ...
            if ($update_all) {
                 ...
                 $groups = $this->groups_model->get_all_group_org_id($org_id);

                 $total_deliveries = (int) $this->overall_reports_model->get_total_deliveries($start_date, $end_date, $org_id);
                 ...
                 $active_day_row = $this->overall_reports_model->get_active_day($start_date, $end_date, $org_id);
                 if ($active_day_row) {
                 ...
                 foreach ($services as $service) {
                      $deliveries = $this->overall_reports_model->overall_performance_service($service->service_id, $start_date, $end_date, $org_id);
                 ...
                 /*
                  * for groups breakdown
                  */
                 $group_breakdown = array();
                 $group_info = array();
                 $group_total = 0;
                 foreach ($groups as $group) {
                      $deliveries = $this->overall_reports_model->overall_performance_group($group->group_id, $start_date, $end_date, $org_id);

                      $d_count = isset($deliveries->deliveries) ? $deliveries->deliveries : 0;
                      $hash = md5($group->group_id . $group->group_name);
                      $R = hexdec(substr($hash, 0, 2));
                      $G = hexdec(substr($hash, 2, 2));
                      $B = hexdec(substr($hash, 4, 2));
                      $group_breakdown[] = array(
                 ...
                 foreach ($group_info as $key => $s) {
                      if ($group_total) {
                           $group_info[$key]['percentage'] = round(($s['value'] / $group_total) * 100);
                      } else {
                           $group_info[$key]['percentage'] = "0";
                      }
                 }
                 /*
                  * end of groups breakdown
                  */
                 ...
                 foreach ($services as $service) {
                      ...
                      $daytrendinfobyservice = $this->overall_reports_model->overall_performance_day_trend_per_service($service->service_id, $start_date, $end_date, $org_id);
