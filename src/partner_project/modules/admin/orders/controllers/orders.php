  ...
  class Orders extends MY_Controller {
       ...
       function __construct() {
            ...
            $this->load->model(array(
                ...
                'couriers_external_model', 'couriers/surcharge_items_model'
            ));
       }
       ...
       public function get_order_json($order_id = NULL) {
            ...
            if ($order_id === NULL) {
                 ...
                 $result['order'] = array(
                     ...
                     'is_c_restricted_area' => FALSE,
                     'is_d_restricted_area' => FALSE,
                     "collect_timezone" => $c_zone,
                     "delivery_timezone" => $d_zone,
                     "collect_country" => $c_country->country ? $c_country->country : 'sg',
                     "delivery_country" => 'sg'
                         // "collect_date" => date("m/d/Y h:m A", now()) . " - " . date("m/d/Y h:m A", now()),
                         // "deliver_date" => date("m/d/Y h:m A", now()) . " - " . date("m/d/Y h:m A", now())
                 );
            } else {
                 $order_id = $this->orders_model->get_order_id($order_id);
                 $where = array(
                     'C.consignment_id' => $order_id,
                     'C.consignment_status_id' => C_DRAFT,
                     'C.is_deleted' => 0
                 );
                 ...
                 if ($order) {
                      ...
                      $service = $this->orders_model->getService($order->service_id);
                      ...
                      $result['order'] = array(
                          ...
                          'is_c_restricted_area' => $order->is_c_restricted_area ? TRUE : FALSE,
                          ...
                          "picture" => $order->picture ? outer_base_url('filebox/orders/' . $order->picture) : "",
                          "remarks" => $order->remarks,
                          "ref" => $order->ref,
                          "tags" => $order->tags
                      );
       ...
       public function saveOrder() {
            ...
            if (isset($orderdata->upload) && !empty($orderdata->upload)) {
                 if (is_file('../filebox/orders/' . $orderdata->upload)) {
       ...
       public function consignment_types() {
            ...
            $result['types'] = $this->consignment_types_model->getList();
            echo json_encode($result);
            exit();
       }
       ...
       public function assigned_services_by_org() {
            ...
            $services = $this->services_model->get_services_assigned($user, $org_id, $where, $status, $type, $consignment_type);
            ...
            foreach ($services as $service) {
                 ...
                 if ($service->time_to_deliver !== null) {
                      ...
                      $str = '+ ' . round($times[0]) . ' days ' . '+ ' . $times[1] . ' hours ';
                      $date = date('Y-m-d H:i:s', strtotime($str));
                      $service->time_to_deliver = $date;
                 }
                 if (isset($d_to) && date('Y-m-d H:i:s', strtotime($d_to)) >= $service->time_to_deliver) {
       ...
       function generate_ciqrcode($file_name = NULL) {
            if (!empty($file_name)) {
                 ...
                 return outer_base_url() . $file_path;
            } else {
                 return '';
            }
       }
       ...
       public function approve_price($public_id = 0, $approve = true) {
            ...
            if ($approve) {
                 ...
                 if ($this->orders_model->updateOrder($data, $job_id)) {
                      ...
                      $result['status'] = 1;
//                      $result['consignment_status'] = "Acceped";
                 ...
                 if ($this->orders_model->updateOrder($data, $job_id)) {
                      ...
                      $result['status'] = 1;
//                      $result['consignment_status'] = "Cancelled";
