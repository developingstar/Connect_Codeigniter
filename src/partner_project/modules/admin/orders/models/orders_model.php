<?php
  ...
  class Orders_model extends CI_Model {
       ...
       public function getorderslist_count($user_id, $search, $org, $service, $status, $team = null, $flag = 0, $excluse_status = array(), $where = NULL) {
            ...
            $this->setorderlist_by_userid($this->db, $user_id, $search, $org, $service, $status, $team, $flag, $excluse_status);
            $query = $this->db->get();
       ...
       public function getorderslist_for_courier($courier_id, $perpage, $search, $start, $org, $service, $status) {
            ...
            $this->db->select('C.consignment_id,C.public_id,C.private_id,C.price,O.name as org_name,U.username,'
                    . 'S.display_name as service,S.price as service_price,C.consignment_status_id,C.collection_address,C.is_c_restricted_area as crestrict,'
       ...
       public function getDetails($where) {
            return $this->db->select('C.*,consignment_type.display_name,F.country as from_country,T.country as to_country,'
                                    ...
                                    . 'S.display_name as service,S.price as service_price,S.payment_terms,Z.courier_id,Z.company_name as courier_name,TP.email as third_party_email,TP.permalink', FALSE)
                            ->from('consignments as C')
       ...
       public function getDetails_print($where) {
            return $this->db->select('C.consignment_id, C.private_id, C.public_id,  C.service_id,C.quantity,'
                                    . 'F.country as from_country,T.country as to_country, '
       ...
       public function getService($id) {
            return $this->db->select("CS.id as service_id,CS.display_name as service_name,CS.description as description,CS.is_public,CS.auto_approve,"
                                    . "CS.org_id,C.courier_id,C.company_name as courier_name,C.logo,CS.threshold_price ")
