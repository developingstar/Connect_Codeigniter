<?php
  ...
  class Service_requests_model extends CI_Model {
       ...
       function getrequestlist_by_org_count($user_id, $org_id, $search) {
            if ($search != NULL) {
       ...
       function getrequestlist_by_org($user_id, $org_id, $perpage, $search, $start) {
            if ($search != NULL) {
                 $this->db->where('(SR.name LIKE \'%' . $search . '%\')');
            }
            ...
            $this->db->from('service_requests as SR');
            $this->db->join('service_bids as SB', 'SB.req_id=SR.req_id', 'left');
       ...
       public function get_request_details($req_id) {
            $this->db->select('req_id, name as title, price_range as price, '
       ...
       public function get_request_list_count($search, $category = NULL) {
            if ($category !== NULL) {
                 if ($category == 1) {
       ...
       public function get_request_list_for_courier($search, $start, $perpage, $category = NULL, $sort = '', $sort_direction = '') {
            ...
            return $this->db->select('req_id as request_id, name, delivery_p_m as deliveries_per_month,'
                                    . ' service_duration , payment_term , other_conditions, remarks as description,SR.status as request_stat, '
                                    . "DATE_FORMAT(added_on,'%d %b %Y %h:%i %p')as added_on, Case When status=1 and expiry_date <='" . date('Y-m-d H:i:s') . "' Then 1
                                        Else 0
