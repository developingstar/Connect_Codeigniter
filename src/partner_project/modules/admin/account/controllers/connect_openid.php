...
class Connect_openid extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->helper(array( 'account/ssl', 'account/openid'));
		...
		if ($this->input->get('janrain_nonce'))
		{
			...
			if ($response->status == Auth_OpenID_SUCCESS)
			{
				...
				if ($user = $this->account_openid_model->get_by_openid($response->getDisplayIdentifier()))
				{
					...
					$user->user_id === $this->session->userdata('partner_user_id') ? $this->session->set_flashdata('linked_error', sprintf(lang('linked_linked_with_this_account'), lang('connect_openid'))) : $this->session->set_flashdata('linked_error', sprintf(lang('linked_linked_with_another_account'), lang('connect_openid')));
				...
				else
				{
					...
					else
					{
						...
						$this->account_openid_model->insert($response->getDisplayIdentifier(), $this->session->userdata('partner_user_id'));
