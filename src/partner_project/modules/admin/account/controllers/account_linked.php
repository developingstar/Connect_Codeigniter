<?php
  ...
  class Account_linked extends MY_Controller {
       ...
       function __construct() {
            ...
            $ci->load->config('account/account');
       ...
       function index() {
            ...
            if ($data['facebook_links'] = $this->account_facebook_model->get_by_user_id($this->session->userdata('partner_user_id'))) {
            ...
            if ($data['twitter_links'] = $this->account_twitter_model->get_by_user_id($this->session->userdata('partner_user_id'))) {
            ...
            // Get OpenID accounts
            if ($data['openid_links'] = $this->account_openid_model->get_by_user_id($this->session->userdata('partner_user_id'))) {
