		...
		if ($this->input->get('janrain_nonce'))
		{
			...
			if ($response->status == Auth_OpenID_SUCCESS)
			{
				...
				if ($user = $this->account_openid_model->get_by_openid($response->getDisplayIdentifier()))
				{
					...
					$user->user_id === $this->session->userdata('partner_user_id') ? $this->session->set_flashdata('linked_error', sprintf(lang('linked_linked_with_this_account'), lang('connect_yahoo'))) : $this->session->set_flashdata('linked_error', sprintf(lang('linked_linked_with_another_account'), lang('connect_yahoo')));
