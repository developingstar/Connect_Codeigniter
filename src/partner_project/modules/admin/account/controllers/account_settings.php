<?php
  ...
  class Account_settings extends MY_Controller {
       ...
       function getAccountDetails() {
            ...
            // Redirect unauthenticated users to signin page
            if ($this->authentication->is_signed_in()) {
                 $account = $this->account_model->get_by_id($this->session->userdata('partner_user_id'));
                 $account_details = $this->account_details_model->get_by_user_id($this->session->userdata('partner_user_id'));
       ...
       function updateSettings() {
            ...
            $errors = array();
            // Retrieve sign in user
            $account = $data['account'] = $this->account_model->get_by_id($this->session->userdata('partner_user_id'));
            $data['account_details'] = $this->account_details_model->get_by_user_id($this->session->userdata('partner_user_id'));
              // Update account details
                      $attributes['phone_no'] = $userinfo->settings_phone ? $userinfo->settings_phone : NULL;
                      $attributes['fax_no'] = $userinfo->settings_fax ? $userinfo->settings_fax : NULL;
