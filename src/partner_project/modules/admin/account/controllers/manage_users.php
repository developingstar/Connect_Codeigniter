  ...
  class Manage_users extends MY_Controller {
       ...
       function index() {
            ...
            foreach ($all_accounts as $acc) {
                 ...
                 foreach ($all_account_roles as $acrole) {
                      if ($acrole->user_id == $acc->id && $acrole->role_id == $admin_role->id) {
       ...
       function save($id = null) {
            ...
            if (!$is_new) {
                 ...
                 $data['update_account_details'] = $this->account_details_model->get_by_user_id($id);
                 $data['update_account_roles'] = $this->acl_role_model->get_by_user_id($id);
                 $data['action'] = 'update';
            }
                 ...
                 elseif ((!empty($id) && strtolower($this->input->post('users_username', TRUE)) != strtolower($data['update_account']->username) && $username_taken) || (empty($id) && $username_taken)) {
                      ...
                      if (empty($id)) {
                           $id = $this->account_model->create(
                                   $this->input->post('users_username', TRUE), $this->input->post('users_email', TRUE), $this->input->post('users_new_password', TRUE));
                      }
                      // Update existing user information
                      else {
                           // Update account username
                           $this->account_model->update_username($id, $this->input->post('users_username', TRUE) ? $this->input->post('users_username', TRUE) : NULL);
