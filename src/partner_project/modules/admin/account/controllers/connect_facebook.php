	...
	function index()
	{
		...
		if ($this->facebook_lib->user)
		{
			...
			if ($user = $this->account_facebook_model->get_by_facebook_id($this->facebook_lib->user['id']))
			{
				...
				$user->user_id === $this->session->userdata('partner_user_id') ? $this->session->set_flashdata('linked_error', sprintf(lang('linked_linked_with_this_account'), lang('connect_facebook'))) : $this->session->set_flashdata('linked_error', sprintf(lang('linked_linked_with_another_account'), lang('connect_facebook')));
			...
			else
			{
				...
				else
				{
					...
					$this->account_facebook_model->insert($this->session->userdata('partner_user_id'), $this->facebook_lib->user['id']);
