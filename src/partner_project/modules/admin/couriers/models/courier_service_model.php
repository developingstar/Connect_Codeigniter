<?php
  ...
  class Courier_service_model extends CI_Model {
       ...
       public function suspend_service($service_id) {
            return $this->db->where('service_id', $service_id)->update("courier_service", array("status" => 2));
       }

       public function suspend_service_by_id($service_id) {
            return $this->db->where('id', $service_id)->update("courier_service", array("status" => 3));
       }

       public function activate_service($service_id) {
            return $this->db->where('id', $service_id)->update("courier_service", array("status" => 1));
       }
       ...
       public function removeservice($service_id, $courier_id) {
            ...
            return $this->db->where($where)->update('courier_service', array("is_for_bidding" => 0, "status" => 2));
       }
       ...
       public function suspendservice($company, $courier_id) {
            $where = array(
                ...
                "courier_id" => $courier_id,
                "status" => 1);
            return $this->db->where($where)->update('courier_service', array("status" => 3));
       }
       ...
       public function activateservice($company, $courier_id) {
            $where = array(
                ...
                "courier_id" => $courier_id,
                "status" => 3);
            return $this->db->where($where)->update('courier_service', array("status" => 1));
       }
       ...
       public function assignservice($service_id, $company) {
            $where = array(
                ...
                "is_for_bidding" => 1,
                "status" => 1);
            return $this->db->where($where)->update('courier_service', array("org_id" => $company, "is_for_bidding" => 0));
       }
       ...
       function getallserviceslist_count($courier_id, $search) {
            ...
            $this->db->where('courier_service.status <> 2');
            $query = $this->db->get();
            return $query->num_rows();
       }

       function getallserviceslist_by_courier($courier_id, $perpage, $search, $start) {
            $this->db->select('CS.id,CS.courier_id,CS.display_name as service,CS.description,CS.is_new,'
                    ...
                    . 'CS.service_id,C.company_name as courier, CS.service_id,'
       ...
       public function get_service_row_id($service_id) {
            $query = $this->db->select('id')
                            ...
                            ->get('courier_service')->row();
            return $query ? $query->id : 0;
       }

       public function get_all_services_for_bidding($courier_id) {
            return $this->db->select('id,service_id,display_name')->from('courier_service')->where('is_for_bidding', 1)->where('status', 1)->where('courier_id', $courier_id)->get()->result();
       }
       ...
       public function get_service_origin($id) {
            return $this->db->select('C.country')->from('courier_service as CS')->join('country AS C', 'C.code=CS.origin', 'left')->where('CS.id', $id)->get()->row();
       }

       public function get_service_destination($d_codes) {
            return $this->db->select('C.country', FALSE)->from('country AS C')->where_in('C.code', $d_codes)->get()->result();
       }
       ...
       public function update_service($id, $data, $where = NULL) {
            ...
            return $this->db->update('courier_service', $data, array('id' => $id));
       }
