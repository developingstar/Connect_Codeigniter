<?php
  ...
  class Courier extends REST_Controller {
       ...
       public function register_post() {
            ...
            $logo = $this->post('logo');
            if (!$email) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No email'), 200);
            } if (!valid_email($email)) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - Not valid email'), 200);
            }
            if ($this->email_check($email)) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - Already Registered Email'), 200);
            }
            if (!$name) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No display name'), 200);
            }
            if (!$password) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No password'), 200);
            }
            if (!$url) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No 6parcels URL'), 200);
            }
//            if (!validate_url($url)) {
...
//            }
            ...
            if ($this->couriers_model->create($name, $email, $password, $token, $url, $description, $logo)) {
                 $this->response(array("code" => $this->config->item('API_OK'), "access_key" => $token), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to register courier"), 200);
       }
...
//            }
            if (!$reg_address) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No registered address'), 200);
            }
            if (!$billing_address) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No billing address'), 200);
            }
            if (!$fullname) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No fullname'), 200);
            }
            if (!$reg_no) {
                 $reg_no = "";
            }
            ...
            if ($this->couriers_model->update_by_access_key($authcode, $data)) {
                 $this->response(array("code" => $this->config->item('API_OK'), "access_key" => $courier->access_key), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to update"), 200);
       }
       ...
       public function getprofile_post() {
            ...
            if ($profile) {
                 $this->response(array("code" => $this->config->item('API_OK'), "courier" => $profile), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => 'Failed to retrieve profile'), 200);
       }
       ...
       public function resetaccesskey_post() {
            ...
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "failed to reset"), 200);
       }
       ...
       public function pushservices_post() {
            ...
            if (!$services) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No services'), 200);
            }
            foreach ($services as $sval) {
                 ...
                 $this->courier_service_model->suspend_service($service_id);
                 if ($this->courier_service_model->push_service($data)) {
//                      if (!$bidding) {
//                           $emails = $this->organisation_model->get_admin_members($company);
//                           $org_name = $this->organisation_model->getname($company);
//                           if ($emails) {
//                                foreach ($emails as $email) {
//                                     $to = $email->email;
//                                     $to_name = $email->username;
//                                     $subject = 'Notification About New Service';
//                                     $message = '<div style="margin:10px;border: 1px solid #eee;background: #fff;padding:50px;width:400px;text-align:center;">'
//                                             . '<h2>New Service \'' . $display_name . '\' Added To \'' . $org_name->name . '\'</h2>'
//                                             . '<p>Please approve/reject the service that is pending approval.</p>'
//                                             . '<a href="' . site_url('system/admin_home#/organisation/' . $company . '/services') . '" style=" background-color: #34495e; border-color: #34495e;color: #ffffff;display:inline-block; width:200px; height:50px;line-height:50px;font-weight: bold;text-align: center;text-decoration: none;">View Service List</a>'
//                                             . '</div>';
...
//                      }
       ...
       public function listservices_post() {
            ...
            if ($services)
                 $this->response(array("code" => $this->config->item('API_OK'), "services" => $services), 200);
            else {
                 $this->response(array("code" => $this->config->item('API_OK'), "services" => 'No services found'), 200);
            }
            $this->response(array("code" => $this->config->item('API_OK'), "access_key" => $authcode), 200);
       }
       ...
       public function removeservice_post() {
            ...
            if (!$service_id) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - no service ID'), 200);
            }
            ...
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "failed to remove this service"), 200);
       }
       ...
       public function suspendservice_post() {
            ...
            if (!$company) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - no company ID'), 200);
            }
       ...
       public function assignservice_post() {
            ...
            if (!$this->organisation_model->is_exist_org($company)) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - invalid company ID'), 200);
            }
            if ($this->courier_service_model->assignservice($service_id, $company)) {
                 $this->response(array("code" => $this->config->item('API_OK'), "access_key" => $authcode), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "failed to assign this service"), 200);
       }
       ...
       public function listjobs_post() {
            ...
            $jobs = $this->orders_model->getjobs($search, $company, $username, $collection_address, $delivery_address, $date_from, $date_to, $ddate_from, $ddate_to, $expired_date, $offset, $limit);
            foreach ($jobs as $job) {
       ...
       public function getjobdetail_post() {
            ...
            if (!$job_id) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid Job ID'), 200);
            }
            ...
            if ($job_details) {
                 ...
                 $this->response(array("code" => $this->config->item('API_OK'), "jobs" => $job_details), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to retrieve job details"), 200);
       }
       ...
       public function bidforjob_post() {
            ...
            if (!$this->orders_model->is_available_for_bid($job_id)) {
                 ...
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'You can\'t bid this job'), 200);
            }
            $service_id = $this->post('service_id');
            if (!$service_id) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No service ID'), 200);
            ...
            if (!$service_row_id) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - Invalid service ID'), 200);
            }
            $price = (float) $this->post('price');
            if (!$price) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid Input - No price'), 200);
            }
            ...
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to bid this job"), 200);
       }
       ...
       public function withdrawbid_post() {
            ...
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to withdraw bid"), 200);
       }
       ...
       public function listbidhistory_post() {
            ...
            if ($jobs) {
                 ...
                 $this->response(array("code" => $this->config->item('API_OK'), "jobs" => 'No jobs found'), 200);
            }

            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to retrieve bid history"), 200);
       }
       ...
       public function listwonjob_post() {
            ...
            $jobs = $this->orders_model->getwonjobs($courier->courier_id, $search, $company, $username, $collection_address, $delivery_address, $date_from, $date_to, $ddate_from, $ddate_to, $expired_date, $offset, $limit);
            foreach ($jobs as $job) {
       ...
       public function listjobmessages_post() {
            ...
            if ($messages) {
                 $this->response(array("code" => $this->config->item('API_OK'), "messages" => $messages), 200);
            } else {
                 $this->response(array("code" => $this->config->item('API_OK'), "messages" => "No messages there"), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to list won jobs"), 200);
       }
       ...
       public function leavejobmessages_post() {
            ...
            if (!$message) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No message'), 200);
            }
            $data = array('courier_id' => $courier->courier_id, 'job_id' => $job_id, 'question' => $message, 'reply' => NULL);
            if ($this->consignment_messages_model->addmessage($data)) {
                 $this->send_mail_for_member($job_id, N_COMMENT_FROM_COURIER);
                 $this->response(array("code" => $this->config->item('API_OK'), "access_key" => $authcode), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to leave message"), 200);
       }
       ...
       public function acknowledgejob_post() {
            ...
            if ($wonbid) {
                 $ack = array('bid_id' => $wonbid->bid_id,
                     'job_id' => $job_id,
            ...
            $data = array('private_id' => $consignment_id,
       ...
       public function adjustprice_post() {
            ...
            if (!$remarks) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid Input - No remarks'), 200);
            }
            $wonjob = $this->job_acknowledgement_model->getwonbid($job_id);
            if ($wonjob) {
                 if ($wonjob->price == $price)
                      $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Same price"), 200);
                 $data = array(
                     "price" => $price,
                     "remarks" => $remarks
                 );
                 if ($this->job_acknowledgement_model->adjustprice($job_id, $data)) {
                      $this->response(array("code" => $this->config->item('API_OK')), 200);
                 }
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to adjust price"), 200);
       }
       ...
       public function listpricehistory_post() {
            ...
            if ($records) {
                 $this->response(array("code" => $this->config->item('API_OK'), "records" => $records), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to retrieve price history"), 200);
       }
       ...
       public function submitjobstate_post() {
            ...
            if (!$status_code) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid Input - No status code'), 200);
            }
            if (!in_array($status_code, array('101', '102', '301', '401', '501', '601'))) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid Input - Unknown status code'), 200);
            }
            $status_name = $this->post('status_name');
            if (!$status_name) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid Input - No status name'), 200);
            }
            $description = $this->post('description');
            if (!$description) {
                 $description = "";
            ...
            if ($o_courier != $courier->courier_id) {
                 $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to submit job state. No permission"), 200);
            }
            ...
            if ($this->jobstates_model->addjobtrack($data)) {
                 $this->orders_model->updateOrder(array('consignment_status_id' => $status_code), $job_id);
                 $this->send_mail_for_member($job_id, N_ORDER_STATUS_UPDATE);
                 $this->response(array("code" => $this->config->item('API_OK')), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to submit job state"), 200);
       }
       ...
       public function checkuserexists_post() {
            ...
            $username = $this->post("username");
            if (!$username) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - no username'), 200);
            }
            $result = $this->account_model->is_exist_user($username);
            $this->response(array("code" => $this->config->item('API_OK'), "exists" => $result), 200);

            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to check user exist"), 200);
       }
       ...
       public function checkcompanyexists_post() {
            ...
            $this->response(array("code" => $this->config->item('API_OK'), "exists" => $exists, "name" => $name), 200);
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to check company exist"), 200);
       }
       ...
       public function send_mail_for_member($order_id = 0, $type = 0) {
                 ...
                 if ($this->notification_model->get_notification($where)) {
                      ...
                      switch ($type) {
                           ...
                           case N_ORDER_STATUS_UPDATE:
                                ...
                                $content = lang('status_update_email_content');
       ...
       public function listservicerequest_post() {
            ...
            if ($requests) {
                 $this->response(array("code" => $this->config->item('API_OK'), "requests" => $requests), 200);
            }
            $this->response(array("code" => $this->config->item('API_OK'), "message" => 'No request found'), 200);
       }
       ...
       public function bidservicerequest_post() {
            ...
            if (!$req_id) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No request ID'), 200);
            }
            if (!$this->service_requests_model->is_available_for_bid($req_id)) {

                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'You can\'t bid this request'), 200);
            }
            $service_id = $this->post('service_id');
            if (!$service_id) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - No service ID'), 200);
            ...
            if ($bid_id) {
                 ...
                 $this->response(array('code' => $this->config->item('API_OK'), 'access_key' => $authcode, 'bid_id' => $bid_id), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to bid this request"), 200);
       }
       ...
       public function withdrawservicerequestbid_post() {
            ...
            $bid_id = (int) $this->post('bid_id');
            if (!$bid_id) {
                 $this->response(array('code' => $this->config->item('API_ERROR'), 'message' => 'Invalid input - no bid ID'), 200);
            }
       ...
       public function listservicerequestbids_post() {
            ...
            if ($bids) {
                 $this->response(array("code" => $this->config->item('API_OK'), "bids" => $bids), 200);
            } else {
                 $this->response(array("code" => $this->config->item('API_OK'), "jobs" => 'No jobs found'), 200);
            }
       ...
       public function listrequestmessages_post() {
            ...
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to list messages"), 200);
       }
       ...
       public function listepreservicerequests_post() {
            ...
            if ($requests) {
                 $this->response(array("code" => $this->config->item('API_OK'), "requests" => $requests, "access_key" => $authcode), 200);
            }
            $this->response(array("code" => $this->config->item('API_OK'), "msg" => "No requests found", "access_key" => $authcode), 200);
       }
       ...
       public function approvepreservice_post() {
            ...
            if ($request) {
                 ...
                 $this->response(array("code" => $this->config->item('API_OK'), "msg" => "Request approved", "access_key" => $authcode), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to approve"), 200);
       }
       ...
       public function rejectpreservice_post() {
            ...
            if ($this->request_courier_service_model->reject_request($req_id, $courier->courier_id)) {
                 $this->response(array("code" => $this->config->item('API_OK'), "msg" => "Request rejected", "access_key" => $authcode), 200);
            }
            $this->response(array("code" => $this->config->item('API_FAILED'), "message" => "Failed to reject"), 200);
       }
