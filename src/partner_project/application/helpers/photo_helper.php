...
function showPhoto($picture = NULL, $param = NULL)
{
	...
	if (isset($picture) && strlen(trim($picture)) > 0)
	{
		...
		if ( ! $remote)
		{
			...
			$path = outer_base_url().'filebox/user/profile/'.$picture; //.		-- disabled time attachment, no need to break cache
		}
		else
		{
		...
		if ($check && ! $remote)
		{
			if ( ! fileExists($path))
			{
				...
				$path =outer_base_url().'resource/images/default-person.png';
			}
		}
