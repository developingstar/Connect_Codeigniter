...
class Authorization {
  ...
  {
    $user_id = $this->CI->session->userdata('partner_user_id');

    $this->CI->load->model('account/acl_permission_model');

    $account_permissions = $this->CI->acl_permission_model->get_by_user_id($user_id);
  ...
  function is_admin()
  {
    ...
    return $this->CI->acl_role_model->has_role('Admin', $user_id);
  }
